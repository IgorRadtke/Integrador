...
try{
    ...
    if(isset($_POST['Email'])&& isset ($_POST['Senha'])){
        ...
        $Nome = ($_POST['Nome']);
        $CRN = ($_POST['CRN']);
        $comando = $pdo->prepare('SELECT crn from nutricionista where crn = :CRN');
        $comando->bindParam(':CRN', $CRN);
        $comando->execute();
        unset( $_SESSION['autorizado'] ); 
        $resultado = $comando->fetchAll(PDO::FETCH_ASSOC);
        if (count($resultado)===0){
            setcookie('crn', 'false', time()+60);
            header('Location: CadastroCli.php');
            exit();
        }
        $DataNasc = ($_POST['DataNasc']);
        $comando = $pdo->prepare('INSERT INTO cliente (codcli, nome, fone, email, senha, datanasc,CRN) VALUES (:CPF, :Nome, :Fone, :Email, :Senha, :DataNasc, :CRN)');
        $comando->bindParam(':CRN', $CRN);
